...
# OpenAlex email for "polite pool" access (faster responses)
...
- OpenAlex API: Open access detection, abstracts, citation counts (240M+ works)
- Semantic Scholar API: Citation counts, AI/ML paper specialization, abstracts
- ArXiv API: Preprint metadata and PDF links
- Google Scholar fallback: Search links for papers not found in other APIs
...
2. OpenAlex for open access detection and broad coverage
3. Semantic Scholar for AI/ML papers and citation metrics
4. ArXiv for preprint identification and PDF access
5. Google Scholar search URL as last resort fallback
6. Robust rate limiting and retry logic for all APIs
...
- `data/`: Input BibTeX files (tracked in git)
- `output/`: Generated feed files (feed.json, feed.xml)
- `cache/`: Metadata and discovery caches (tracked in git for persistence)
- `logs/`: Application logging
- `src/`: Main application modules
- `tests/`: Test suite (91 tests across 5 files)
- `config.yml`: Central configuration file (supports env vars)

## Testing Approach

The test suite covers all core components with 91 tests:
...
- `test_openalex.py`: OpenAlex client, abstract reconstruction, response parsing
